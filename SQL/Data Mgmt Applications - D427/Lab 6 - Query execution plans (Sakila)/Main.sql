...
/* Analyzes query execution plan for selecting actors and their average film length for a specific title */
EXPLAIN
SELECT last_name, first_name, ROUND(AVG(length), 0) AS average
FROM actor
INNER JOIN film_actor ON film_actor.actor_id = actor.actor_id
...
/* Checks query execution plan when filtering for titles alphabetically before "ALONE TRIP" */
EXPLAIN
SELECT last_name, first_name, ROUND(AVG(length), 0) AS average
FROM actor
INNER JOIN film_actor ON film_actor.actor_id = actor.actor_id
...
/* Checks query execution plan when filtering for titles alphabetically after "ALONE TRIP" */
EXPLAIN
SELECT last_name, first_name, ROUND(AVG(length), 0) AS average
FROM actor
INNER JOIN film_actor ON film_actor.actor_id = actor.actor_id
